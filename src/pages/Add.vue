  ...
  <div class="wrapper">
    ...
    <div class="search">
      ...
      <input type="text"
        ...
        placeholder="在这边输入用户名或者群名称哦" />
    </div>
    <div class="choose">
      <p v-show="this.inputContent !='' "
        class="find-people"
        @click="findPeople">
        <svg class="icon"
          aria-hidden="true">
          <use xlink:href="#icon-my-copy"></use>
        </svg><span>加人:&nbsp&nbsp{{this.inputContent}}</span>
      </p>
      <p v-show="this.inputContent !='' "
        class="find-group"
        @click="findGroup"><svg class="icon"
          aria-hidden="true">
          <use xlink:href="#icon-group1"></use>
        </svg><span>进群:&nbsp&nbsp{{this.inputContent}}</span>
      </p>
